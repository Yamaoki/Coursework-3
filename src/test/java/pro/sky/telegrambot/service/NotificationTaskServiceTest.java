...
class NotificationTaskServiceTest {
    ...
    void createNotificationTaskTest() {
        ...
        task1.setChatId(1L);
        LocalDateTime dateTime = LocalDateTime.parse("2023-08-18 18:10:00", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        task1.setTaskTime(dateTime);
        task1.setTaskText("Текст задания");

        when(repository.save(task1)).thenReturn(task1);

        assertThat(task1).isSameAs(service.create(1L, "18.08.2023 18:10", "Текст задания"));
    ...
    void getNotificationTasksByTaskTime() {
        LocalDateTime dateTime = LocalDateTime.parse("2023-08-18 18:15:00", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
